...
**Step 1.2 — API Key Authentication** The request header must contain a valid API key. The system looks up the API key in the users table. If not found or inactive, return 401 Unauthorized immediately. If found, the user object is attached to the request context for the rest of the flow.
